...
| **One ACTIVE SCHEDULED invoice per enrollment episode** | a *set*-based invariant — at most one non-void **scheduled** (term-bill) invoice per episode, re-billable after a void. **SUPPLEMENTARY invoices are unconstrained** — any number, at any time, for the same episode | STORED generated column `active_enrollment_key` + `UNIQUE(school_id, active_enrollment_key)`, keyed on `status = 'issued' AND kind = 'scheduled'` since `2026_08_18_100000` (F7 in `docs/roadmap.md` quotes the expression as `information_schema` reports it). Generated, not app-maintained; `kind` is immutable at the DB |
