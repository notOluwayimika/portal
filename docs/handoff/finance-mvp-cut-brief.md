...
| **Phase 1 substrate** | 12 of 17 §4.4 debt items ✅. `ActiveSchool`, `SchoolAccessParity`, `SchoolScope` fail-closed (built), `PermissionCatalog`, `EffectivePermissions`, `RouteAccessMap`, grants-convergence lint, duty-separation audit, six content-keyed ratchet baselines (ADR 0041), `bin/quality` 13 steps + `.githooks/pre-push`. |
...
| S10 | **Refund as an approval-gated cash-out posting** — one path, one modal, no bulk | Ph7 | Money leaving the building must be in the ledger or the ledger is wrong. Low frequency, but the shape must exist before the first one happens. Needs a third state (flagged earlier, still open). |
...
| U12b | **✱ Bulk opening-balance import** — upload · staged preview with row rejects and a control total · approve · post. Posts opening entries, **never payments** | V11, S9 | **Nothing.** *Moved in from OUT 2026-08-06.* |
| **Corrections** ||||
...
| U20 | **Year preview per ward** — the whole academic year's fees, per term and totalled, labelled an estimate | V10 | **Nothing.** Not in v10 at all. |
...
**Fourteen of twenty-four are nothing at all today** (nine staff + the four parent screens, minus overlap: U2, U3, U5, U8, U10, U11, U12, U12b, U15, U18, U20, U21, U22, U23). **Two of the surfaces that exist are single-student modals where the MVP needs a bulk or an allocation surface.**
...
4. **Are the fee schedules for all three terms going to exist before term-1 billing?** V10's year preview is blank without them. This is an operating commitment from Brookstone Finance, not a build item.
5. **Has a refund been issued in the last three terms?** Still unasked, still worth S10.
...
- **Refunds (S10) may not fire in term one at all.** If Brookstone confirms no refund has been issued in the last three terms, S10 drops to the OUT column and takes the third-state design question with it. I have not asked.
