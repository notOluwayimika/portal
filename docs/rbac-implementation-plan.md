...
| C3 | `feat/rbac-policies` | **SHIPPED 2026-07-21.** Delivered: **ADR 0044 implemented** (4 FormRequests → permissions; dormant controller checks → permissions in observe mode; steps 2/3/5) + **ADR 0040 implemented** — bypass exclusion as a terminal-segment convention, and structural maker ≠ checker at Policy **and** DB (a schema change: the result-status table's single overwritten `updated_by` made the rule unrepresentable) + `SubjectResultPolicy` on the ExportPolicy pattern with explicit `Gate::policy` + the deferred role-gate re-audit closed. **Deviation from this row:** the Guardian/Student/Activity policies were *not* built — those surfaces are authorized today by the 45 `Authz` call sites **A6 owns**, so writing policies for them here would either duplicate a live gate or pre-empt A6's migration. Carried to **C3b**, to run after or with A6. PrincipalController's `hasRole` deliberately **stays** (target identity, not caller authorization). | No | C1; **sequence against Finance's enrollment-controller slices — never concurrent** ⚑ |
