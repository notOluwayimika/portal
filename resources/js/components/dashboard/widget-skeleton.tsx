...
export function WidgetSkeleton({ variant = 'kpi' }: WidgetSkeletonProps) {
    if (variant === 'chart') {
        return (
            <div className="bg-white border border-slate-200 rounded-lg p-4 animate-pulse dark:bg-slate-900 dark:border-slate-700">
    ...
    if (variant === 'list') {
        return (
            <div className="bg-white border border-slate-200 rounded-lg p-4 animate-pulse space-y-3 dark:bg-slate-900 dark:border-slate-700">
                <div className="h-3 bg-slate-100 rounded w-1/3 mb-4" />
                {[1, 2, 3, 4].map((i) => (
