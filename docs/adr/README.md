...
| [0051](0051-approval-requirement-is-configuration.md) | "Does this need a second signature" is configuration behind one seam (`ApprovalRequirement`); fails closed, changes no behaviour; not a polymorphic engine | 1 | **Accepted** |
| [0052](0052-a-migration-is-a-dated-act.md) | A migration is a dated act, not a live query: grant targets are frozen literals, never read from the seeder map; a convergence migration aborts only on a condition its own writes would create; **corollary — an applied migration is never edited in place** (roll back first, or ship a new dated one) | 1 | **Accepted** |
| [0053](0053-local-enforcement-floor.md) | The enforcement floor is local, permanently: `bin/quality` plus `.githooks/pre-push` are the gate, not a stopgap for CI; what fifteen steps guarantee, and the four residuals they cannot | 1→Continuous | **Accepted** |
| [0054](0054-one-server-side-money-formatter.md) | One server-side money formatter on the value object (`Money::format()`): symbol not ISO code, float-free grouping by string surgery, NGN-only; kobo integers retained in diagnostics; money-lint gains a PHP arm | 1 | **Accepted** |
