...
function loginBadge(guardian: Guardian) {
    ...
    if (guardian.has_login) {
        return (
            <span
                className={`${base} bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400`}
            >
                Enabled
            </span>
        );
    ...
    return (
        <span
            className={`${base} bg-gray-100 text-gray-600 dark:bg-gray-800 dark:text-gray-400`}
        >
            No Account
        </span>
    );
...
/**
 * Sortable header cell. Hoisted to module scope deliberately: it was defined
 * INSIDE GuardianTable's body, so React saw a brand-new component type on every
 * render and remounted all six header cells each time (react-hooks
 * "Cannot create components during render"). Same markup, same behaviour — the
 * three values it used to close over are now props.
 */
function SortBtn({
    col,
    label,
            ...
            <table className="w-full text-xs">
                <thead>
                    <tr className="border-b border-slate-100 bg-slate-50/50 dark:border-slate-800 dark:bg-slate-900/30">
                        ...
                        <SortBtn col="name" label="Name" {...sortProps} />
                        <SortBtn col="phone" label="Phone" {...sortProps} />
                        <th className="hidden px-3 py-2 text-left text-[10px] font-bold tracking-wide whitespace-nowrap text-slate-400 uppercase sm:table-cell">
                            Email
                        </th>
                        <SortBtn
                            col="students_count"
                            ...
                            {...sortProps}
                        />
                        <SortBtn col="login" label="Login" {...sortProps} />
                        <th className="px-3 py-2 text-left text-[10px] font-bold tracking-wide whitespace-nowrap text-slate-400 uppercase">
                            Status
                        </th>
                        <SortBtn
                            col="created_at"
                            ...
                            {...sortProps}
                        />
                        <th className="px-3 py-2 text-right text-[10px] font-bold tracking-wide whitespace-nowrap text-slate-400 uppercase">
                            Actions
                        </th>
                    </tr>
                </thead>
                        ...
                        guardians.map((g) => (
                            ...
                            >
                                ...
                                <td className="hidden px-3 py-2.5 text-muted-foreground sm:table-cell">
                                    {g.email &&
                                    !g.email.endsWith('@no-email.local')
                                        ? g.email
                                        : '—'}
                                </td>
