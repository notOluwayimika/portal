...
/**
 * Per-School invoice-number prefix — PRESENTATION-DERIVED, never stored.
 *
 * The prefix is composed on the way out; `finance_invoices.number` stays the integer
 * that `UNIQUE(school_id, number)` and the Sequences kernel depend on. These tests
 * pin both halves: that the rendered form is right, AND that nothing was stored.
 */
...
it('renders the configured prefix, and the bare number when none is configured', function () {
    ...
    // The policy's own defaults carry their separator: BSS-, BSP-, BSI-LAG-.
    SchoolFinanceSettings::create([
        'school_id' => $configured->id,
        'invoice_number_prefix' => 'BSS-',
    ]);

    expect(prefixInvoice($configured, 42)->displayNumber())->toBe('BSS-42')
        // No settings row at all — absence of configuration is a valid state, so the
        // invoice keeps its bare number. This is every School on the day this lands.
        ->and(prefixInvoice($unconfigured, 42)->displayNumber())->toBe('42');
});
...
it('treats a blank prefix as unset — no invisible difference from the bare number', function () {
    ...
    expect(prefixInvoice($school, 7)->displayNumber())->toBe('7');
});

it('concatenates verbatim — no separator injected, no zero-padding invented', function () {
    // `BSI-LAG-` is the policy's own third default and it contains an INTERNAL hyphen.
    // Any "append a dash" logic renders `BSI-LAG--9`; any width rule renders `BSI-LAG-00009`.
    // The policy specifies neither, so neither is invented.
    $school = School::factory()->create();
    SchoolFinanceSettings::create(['school_id' => $school->id, 'invoice_number_prefix' => 'BSI-LAG-']);

    expect(prefixInvoice($school, 9)->displayNumber())->toBe('BSI-LAG-9')
        ->and(prefixInvoice($school, 12345)->displayNumber())->toBe('BSI-LAG-12345');
});
...
it('PER-SCHOOL ISOLATION — each School renders its own prefix, never another\'s', function () {
    ...
    expect($invoiceA->displayNumber())->toBe('BSS-100')
        ->and($invoiceB->displayNumber())->toBe('BSP-100')
        ...
        ->and($invoiceA->displayNumber())->toBe('BSS-100');
});
