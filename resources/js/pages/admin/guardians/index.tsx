...
export default function GuardianIndex({ guardian_statuses }: Props) {
    ...
    useEffect(() => {
        ...
        const filterChanged =
            ...
            prev.loginFilter !== loginFilter ||
            prev.childrenFilter !== childrenFilter ||
    ...
    const toggleAll = (checked: boolean) => {
        setSelectAllMatching(false);
        setSelectedIds(
            checked ? new Set(guardians.map((g) => g.id)) : new Set(),
        );
    };
    ...
    const handleExport = () => {
        const params = new URLSearchParams({
            search,
            status: statusFilter,
            login_access: loginFilter,
            children_count: childrenFilter,
            date_from: dateFrom,
            date_to: dateTo,
        });
    ...
    return (
        <>
                            ...
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs text-slate-500">
                                        Manage guardian accounts, login access,
                                        and student links.
                                    </p>
                                </div>
                            </div>
            ...
            <BulkActionBar
                ...
                onEnableLogin={() =>
                    bulkPost('/api/guardians/bulk-enable-login')
                }
                onDisableLogin={() =>
                    bulkPost('/api/guardians/bulk-disable-login')
                }
                onChangeStatus={(s) =>
                    bulkPost('/api/guardians/bulk-status', { status: s })
                }
            />
