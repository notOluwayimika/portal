...
it('account payment "USD" is a 422 and no balance moves, no payment, no ledger row', function () {
    ...
    $this->actingAs($u)->withSession(['school_id' => $s->id])->postJson($url, ['amount_minor' => 100000, 'currency' => 'NGN', 'payer_name' => 'X'])->assertCreated();
    ...
    $this->actingAs($u)->withSession(['school_id' => $s->id])
        ->postJson($url, ['amount_minor' => 100000, 'currency' => 'USD', 'payer_name' => 'X'])
...
it('invoice payment "USD" is a 422 on both arms (outstanding>0 and fully allocated)', function () {
    ...
    $post = fn (int $amt, string $cur) => $this->actingAs($u)->withSession(['school_id' => $s->id])
        ->postJson("/api/v1/finance/invoices/{$inv->uuid}/payments", ['amount_minor' => $amt, 'currency' => $cur, 'payer_name' => 'X']);
...
it('NGN and an omitted currency still record on both routes; a fully-allocated NGN advance banks as credit', function () {
    ...
    $this->actingAs($u)->withSession(['school_id' => $s->id])->postJson($acctUrl, ['amount_minor' => 100000, 'currency' => 'NGN', 'payer_name' => 'X'])->assertCreated();
    $this->actingAs($u)->withSession(['school_id' => $s->id])->postJson($acctUrl, ['amount_minor' => 50000, 'payer_name' => 'X'])->assertCreated();
    ...
    $this->actingAs($u)->withSession(['school_id' => $s->id])->postJson($invUrl, ['amount_minor' => 100000, 'currency' => 'NGN', 'payer_name' => 'X'])->assertCreated();
    $this->actingAs($u)->withSession(['school_id' => $s->id])->postJson($invUrl, ['amount_minor' => 50000, 'payer_name' => 'X'])->assertCreated();
});
...
it('SubledgerPoster refuses a currency mismatch against an existing account (LogicException)', function () {
    ...
    $this->actingAs($u)->withSession(['school_id' => $s->id])
        ->postJson("/api/v1/finance/students/{$student->uuid}/payments", ['amount_minor' => 100000, 'currency' => 'NGN', 'payer_name' => 'X'])->assertCreated();
    ...
    ActiveSchool::runFor($s->id, function () use ($student, $u) {
        expect(fn () => app(RecordAccountPayment::class)->handle($student->id, Money::fromKobo(100000, 'USD'), 'X', $u))
