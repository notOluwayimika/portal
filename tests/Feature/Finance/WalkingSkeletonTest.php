...
{
    return (int) DB::table('fee_ledger_transactions')->where('student_id', $studentId)->sum('amount_minor');
}
...
it('cancels by REVERSAL: invoice row persists, status flips, ledger nets to zero', function () {
    ...
    $create = $this->actingAs($admin)->withSession(['school_id' => $school->id])
        ->postJson('/api/finance/invoices', ['enrollment_id' => $enrollment->uuid, 'amount_minor' => 150000, 'description' => 'Tuition'])
        ->assertCreated();
    $invoiceUuid = $create->json('id');

    $this->actingAs($admin)->withSession(['school_id' => $school->id])
        ->postJson("/api/finance/invoices/{$invoiceUuid}/cancel", ['reason' => 'entered in error'])
        ->assertOk()
    ...
    expect($invoice)->not->toBeNull()                                  // never deleted
        ...
        ->and(DB::table('fee_ledger_transactions')->count())->toBe(2); // both entries survive (append-only)
});
...
it('records a payment allocated to the invoice, crediting the ledger to zero', function () {
    ...
    $this->actingAs($admin)->withSession(['school_id' => $school->id])
        ->postJson("/api/finance/invoices/{$invoiceUuid}/payments", ['amount_minor' => 150000, 'payer_name' => 'Mr Obi'])
        ->assertCreated()
        ->assertJsonPath('amount.amount_minor', 150000);

    expect(DB::table('fee_payment_allocations')->count())->toBe(1)
...
it('GUARD — a payment allocation survives invoice cancellation, leaving a credit balance', function () {
    [$school, $admin, $enrollment] = financeSetup();
    $create = $this->actingAs($admin)->withSession(['school_id' => $school->id])
        ->postJson('/api/finance/invoices', ['enrollment_id' => $enrollment->uuid, 'amount_minor' => 150000, 'description' => 'Tuition'])->assertCreated();
    $invoiceUuid = $create->json('id');
    $this->actingAs($admin)->withSession(['school_id' => $school->id])
        ->postJson("/api/finance/invoices/{$invoiceUuid}/payments", ['amount_minor' => 150000, 'payer_name' => 'Mr Obi'])->assertCreated();

    $this->actingAs($admin)->withSession(['school_id' => $school->id])
        ->postJson("/api/finance/invoices/{$invoiceUuid}/cancel", ['reason' => 'error'])->assertOk();
...
it('GUARD — cancelling an already-cancelled invoice is rejected', function () {
    [$school, $admin, $enrollment] = financeSetup();
    $create = $this->actingAs($admin)->withSession(['school_id' => $school->id])
        ->postJson('/api/finance/invoices', ['enrollment_id' => $enrollment->uuid, 'amount_minor' => 5000, 'description' => 'x'])->assertCreated();
    $invoiceUuid = $create->json('id');
    $this->actingAs($admin)->withSession(['school_id' => $school->id])
        ->postJson("/api/finance/invoices/{$invoiceUuid}/cancel", ['reason' => 'a'])->assertOk();

    $this->actingAs($admin)->withSession(['school_id' => $school->id])
        ->postJson("/api/finance/invoices/{$invoiceUuid}/cancel", ['reason' => 'b'])
        ->assertStatus(422);
});
...
it('GUARD — ON DELETE RESTRICT: once an invoice references it, the enrollment/curriculum cannot be cascaded away', function () {
    ...
    // curricula ← student_curricula is CASCADE; fee_invoices ← student_curricula is
...
it('GUARD — the subledger is append-only: raw UPDATE and DELETE are denied at the DB', function () {
    ...
    $rowId = DB::table('fee_ledger_transactions')->value('id');

    // Triggers fire even against raw DB writes (what tinker / a mass delete would do).
    expect(fn () => DB::table('fee_ledger_transactions')->where('id', $rowId)->update(['amount_minor' => 1]))
        ->toThrow(QueryException::class)
        ->and(fn () => DB::table('fee_ledger_transactions')->where('id', $rowId)->delete())
        ->toThrow(QueryException::class);
});
