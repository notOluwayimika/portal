...
| 7 | `SchoolScope` fails open | ◑ | Fail-closed mechanism built (throws `MissingSchoolContextException`), rolled out **per model** via `config('rbac.fail_closed_models')`. As of 2026-08-09 the ten finance TRANSACTIONAL models are opted in as a versioned default; the finance **catalog** models and everything outside Finance still fail open. Remains partial for a second reason: the throw is gated on `auth()->check()`, so **no off-request path is protected at all** — see docs/handoff/reports/feat-rbac-fail-closed-finance.md |
| 8 | 6 public API endpoints | ✅ | Duplicate declarations moved inside the `auth:sanctum` group in `routes/api.php` |
| 9 | CI has never passed | ⬜ **STILL OPEN — my ✅ was WRONG (corrected 2026-07-20)** | I marked this resolved on the evidence that the *machinery* is wired (`.env.example` present, `phpunit*.xml` pinned to `portal_testing`, `composer ci:check` + `lint.yml`/`tests.yml` defined, ratchet baselines committed). Wired ≠ executed. **GitHub Actions has never run a single job on this repo: the account is billing-locked.** Every run is annotated *"The job was not started because your account is locked due to a billing issue"* — `steps=0`, ~4s duration. So CI has neither passed nor failed; there is no pipeline result at all, and nothing to debug until billing is resolved. Separately, the checks are **not required status checks** — PRs #56/#57/#58 all merged over red. **Interim:** `bin/quality` (mirrors both workflows step-for-step) enforced by a pre-push hook is the gate; see CLAUDE.md § Workflow for the exit trigger back to CI. |
