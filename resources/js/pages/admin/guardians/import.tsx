...
import { guardianImports } from '@/services/guardian-imports';
import type { GuardianImportRecord } from '@/services/guardian-imports';

type Row = Record<string, unknown>;

const REQUIRED_COLUMNS = GUARDIAN_IMPORT_COLUMNS.filter((c) => c.required).map(
    (c) => c.name,
);
...
export default function GuardianImport() {
    ...
    const progressPct =
        active && active.total_rows > 0
            ? Math.min(
                  100,
                  Math.round((active.processed_rows / active.total_rows) * 100),
              )
            : 0;

    // DECLARED ABOVE ITS FIRST USE. `react-hooks/immutability` flagged this as accessed
    // before declaration: the mount effect below called it while the `const` was still in
    // its temporal dead zone at module-evaluation order. It worked only because the effect
    // runs after render; moving the declaration removes the trap rather than the symptom.
    // A PROMISE CHAIN, NOT async/await — CAUSE 2 of the two recorded in
    // resources/js/pages/admin/internal-audit/review-queue.tsx. With
    // `try { await … } catch { … }` the catch CAN RUN IN THE SAME TICK: if the awaited
    // expression throws before it yields a promise, control arrives synchronously, so a
    // setState reachable from here is reachable synchronously from the effect that calls it.
    // A promise continuation is always scheduled, so `setRecent` is unreachable synchronously
    ...
    // KEYED ON PRIMITIVES, NOT ON THE OBJECT. `react-hooks/exhaustive-deps` wants `active` in
    // the dep list; adding it would tear down and recreate this interval ON EVERY POLL,
    // because each poll replaces `active` with a fresh object — a silent change to the
    // cadence this screen depends on. Reading the two fields the effect actually uses into
    // locals means it closes over exactly what it is keyed on: same guard, same 2s interval,
    // no re-creation.
    ...
    useEffect(() => {
        ...
        const id = setInterval(async () => {
            try {
                const fresh = await guardianImports.status(activeUuid);
                setActive(fresh);
                ...
                if (fresh.status === 'completed' || fresh.status === 'failed') {
                    ...
                    if (fresh.status === 'completed') {
                        toast.success(
                            `Import complete: ${fresh.succeeded} succeeded, ${fresh.failed} failed, ${fresh.skipped} skipped.`,
                        );
                    } else {
                        toast.error(
                            `Import failed: ${fresh.error ?? 'unknown error'}`,
                        );
                    }
                }
            } catch {
    ...
    const handleSubmit = async () => {
        ...
        try {
            const imported = await guardianImports.submit(
                selectedFile,
                updateExistingLinks,
            );
            setActive(imported);
            ...
            if (imported.status === 'completed') {
                toast.success(
                    `Import complete: ${imported.succeeded} succeeded, ${imported.failed} failed, ${imported.skipped} skipped.`,
                );
            } else {
                toast.message(
                    'Import queued. We will email you when it finishes.',
                );
            }
        } catch (err: any) {
            const message =
                    ...
                    <div className="relative overflow-hidden rounded-2xl border border-white bg-white px-6 py-4 shadow-[0_8px_30px_rgb(0,0,0,0.04)] dark:border-white/5 dark:bg-card">
                        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <p className="max-w-xl text-xs text-slate-500">
                                        Onboard many guardians at once.
                                        Duplicates are detected by phone or
                                        email, so the same parent across
                                        siblings is linked, not duplicated.
                                    </p>
                                </div>
                            </div>
                            ...
                            <div className="flex shrink-0 flex-wrap items-center gap-2">
                                <a
                                    href={guardianImports.templateUrl()}
                                    download
                                >
                                    <Button
                                        size="sm"
                    ...
                    <div className="flex items-center gap-2.5 rounded-xl border border-amber-200 bg-amber-50 px-4 py-2.5 text-xs text-amber-800 dark:border-amber-500/30 dark:bg-amber-950/30 dark:text-amber-200">
                        ...
                        <span>
                            <span className="font-semibold">
                                Photos are not part of bulk import.
                            </span>{' '}
                            Add them individually on each guardian's profile
                            page after the import completes.
                        </span>
                    </div>
                                ...
                                {active.status === 'completed' && (
                                    <>
                                        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                                            ...
                                            <SummaryStat
                                                label="Skipped"
                                                value={active.skipped}
                                                accent="amber"
                                            />
                                        </div>
                                        {active.has_report && (
                                            <a
                                                href={guardianImports.reportUrl(
                                                    active.uuid,
                                                )}
                            ...
                            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                                <label className="flex items-start gap-2.5 rounded-lg bg-slate-50/70 px-3 py-2 text-xs dark:bg-slate-900/40">
                                    <Checkbox
                                        ...
                                        onCheckedChange={(v) =>
                                            setUpdateExistingLinks(Boolean(v))
                                        }
                                        className="mt-0.5"
                                    />
                                    <span>
                                        <span className="block text-sm font-semibold text-slate-700 dark:text-slate-200">
                                            Update existing student-guardian
                                            links
                                        </span>
                                        <span className="block text-xs text-slate-500">
                                            Refresh relationship, primary flag,
                                            and login flag from the file when a
                                ...
                                >
                                    ...
                                    {previewData.length > 0
                                        ? `Import ${previewData.length} Row(s)`
                                        : 'Import'}
                                </Button>
                            </div>
                                        ...
                                        <thead className="sticky top-0 z-10 bg-slate-50/80 backdrop-blur-sm dark:bg-slate-900/80">
                                            <tr>
                                                ...
                                                {PREVIEW_COLUMNS.map((c) => (
                                                    <th
                                                        key={c.key}
                                                        className="px-3 py-2 text-left text-[10px] font-bold tracking-wide text-slate-400 uppercase"
                                                    >
                                                        {c.label}
                                                    </th>
                                                ))}
                            ...
                            {recent.length === 0 ? (
                                <div className="flex flex-col items-center justify-center py-10 text-center">
                                    ...
                                    <p className="mt-1 max-w-[280px] text-xs text-slate-500">
                                        Imports you run will appear here with
                                        status, counts, and a report you can
                                        re-download.
                                    </p>
                                </div>
                            ) : (
                                <div className="custom-scrollbar overflow-x-auto">
                                    <table className="w-full text-xs">
                                        <thead className="bg-slate-50/50 dark:bg-slate-900/30">
                                            <tr>
                                                <th className="px-4 py-2 text-left text-[10px] font-bold tracking-wide text-slate-400 uppercase">
                                                    File
                                                </th>
                                                <th className="px-4 py-2 text-left text-[10px] font-bold tracking-wide text-slate-400 uppercase">
                                                    Date
                                                ...
                                                <th className="px-4 py-2 text-right text-[10px] font-bold tracking-wide text-slate-400 uppercase">
                                                    Succeeded
                                                </th>
                                                <th className="px-4 py-2 text-right text-[10px] font-bold tracking-wide text-slate-400 uppercase">
                                                    Failed
                                        ...
                                        <tbody>
                                            {recent.map((r) => (
                                                <tr
                                                    key={r.uuid}
                                                    className="border-t border-slate-100 hover:bg-slate-50/40 dark:border-slate-800 dark:hover:bg-slate-900/30"
                                                >
                                                    ...
                                                    <td className="px-4 py-2.5">
                                                        <Badge
                                                            className={cn(
                                                                'inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[10px] font-semibold capitalize shadow-sm',
                                                                statusBadge(
                                                                    r.status,
                                                                ),
                                                            )}
            ...
            >
                <div className="space-y-3">
                    <div className="flex items-start gap-3 rounded-xl border border-indigo-200 bg-indigo-50 p-3 text-xs text-indigo-800 dark:border-indigo-500/30 dark:bg-indigo-950/30 dark:text-indigo-200">
                        ...
                        <span>
                            Required columns must be filled on every row.
                            Optional columns can be left blank. Column headers
                            are matched case-insensitively and spaces are
                            converted to underscores.
                        </span>
                    </div>
                    <div className="custom-scrollbar max-h-[60vh] overflow-auto rounded-xl border border-slate-100 dark:border-slate-800">
                        <table className="w-full text-sm">
                            <thead className="sticky top-0 bg-slate-50/80 backdrop-blur-sm dark:bg-slate-900/80">
                                <tr>
                                    <th className="px-3 py-2 text-left text-xs font-bold tracking-wide text-slate-400 uppercase">
                                        Column
                                    </th>
                                    <th className="px-3 py-2 text-left text-xs font-bold tracking-wide text-slate-400 uppercase">
                                        Group
