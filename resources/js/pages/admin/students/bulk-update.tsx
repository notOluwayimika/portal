...
import { studentBulkUpdate } from '@/services/student-bulk-update';
import type { StudentBulkUpdateRecord } from '@/services/student-bulk-update';

type Row = Record<string, unknown>;

const REQUIRED_COLUMNS = STUDENT_BULK_UPDATE_COLUMNS.filter(
    (c) => c.required,
).map((c) => c.name);
...
export default function StudentBulkUpdate() {
    ...
    useEffect(() => {
        ...
        const id = setInterval(async () => {
            try {
                const fresh = await studentBulkUpdate.status(activeUuid);
                setActive(fresh);
                ...
                if (fresh.status === 'completed' || fresh.status === 'failed') {
                    ...
                    if (fresh.status === 'completed') {
                        toast.success(
                            `Update complete: ${fresh.succeeded} updated, ${fresh.failed} failed, ${fresh.skipped} skipped.`,
                        );
                    } else {
                        toast.error(
                            `Update failed: ${fresh.error ?? 'unknown error'}`,
                        );
                    }
                }
            } catch {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (imported.status === 'completed') {
                toast.success(
                    `Update complete: ${imported.succeeded} updated, ${imported.failed} failed, ${imported.skipped} skipped.`,
                );
            } else {
                    ...
                    <div className="relative overflow-hidden rounded-2xl border border-white bg-white px-6 py-4 shadow-[0_8px_30px_rgb(0,0,0,0.04)] dark:border-white/5 dark:bg-card">
                        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <p className="max-w-xl text-xs text-slate-500">
                                        ...
                                        using a spreadsheet. Students are
                                        matched by their admission number (Code
                                        column). Only non-empty columns are
                                        updated.
                                    </p>
                                </div>
                            </div>
                            ...
                            <div className="flex shrink-0 flex-wrap items-center gap-2">
                                <a
                                    href={studentBulkUpdate.templateUrl()}
                                    download
                                >
                                    <Button
                                        size="sm"
                    ...
                    <div className="flex items-center gap-2.5 rounded-xl border border-blue-200 bg-blue-50 px-4 py-2.5 text-xs text-blue-800 dark:border-blue-500/30 dark:bg-blue-950/30 dark:text-blue-200">
                        ...
                        <span>
                            <span className="font-semibold">
                                Only filled columns are updated.
                            </span>{' '}
                            Leave a column blank to keep the existing value. The
                            "Code" column is required to identify each student.
                            After the update, download the report to see which
                            rows failed and why.
                        </span>
                    </div>
                            ...
                            <CardContent className="space-y-4 p-5">
                                ...
                                {active.status === 'completed' && (
                                    <>
                                        ...
                                        {active.has_report && (
                                            <a
                                                href={studentBulkUpdate.reportUrl(
                                                    active.uuid,
                                                )}
                            ...
                            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-end">
                                ...
                                >
                                    ...
                                    {previewData.length > 0
                                        ? `Update ${previewData.length} Student(s)`
                                        : 'Update'}
                                </Button>
                            </div>
                            ...
                            {recent.length === 0 ? (
                                <div className="flex flex-col items-center justify-center py-10 text-center">
                                    ...
                                    <p className="mt-1 max-w-[280px] text-xs text-slate-500">
                                        Bulk updates you run will appear here
                                        with status, counts, and a downloadable
                                        report.
                                    </p>
                                </div>
                            ) : (
            ...
            >
                <div className="space-y-3">
                    <div className="flex items-start gap-3 rounded-xl border border-indigo-200 bg-indigo-50 p-3 text-xs text-indigo-800 dark:border-indigo-500/30 dark:bg-indigo-950/30 dark:text-indigo-200">
                        ...
                        <span>
                            Only the <strong>Code</strong> column is required.
                            All other columns are optional &mdash; leave them
                            blank to keep the existing value. Column headers are
                            matched case-insensitively and spaces are converted
                            to underscores.
                        </span>
                    </div>
